...
auto-discovers the Pi, and forwards the trajectory via TCP.

Usage: python socket/mac_spline_server.py
"""
...
# Path to the tests/ directory where spline_editor.html lives
...
def send_trajectory_to_pi(trajectory):
    ...
    if pi_ip is None:
        ...
        if not discover_pi(timeout=5):
            raise ConnectionError("Could not find Pi on the network. Is pi_spline_motor.py running?")
    ...
    payload = json.dumps(trajectory).encode('utf-8')
    header = struct.pack('>I', len(payload))
    sock.sendall(header + payload)

    print(f"[NET] Sent {len(trajectory)} points ({len(payload)} bytes) to Pi.")
        ...
        if self.path == '/trajectory':
            ...
            try:
                ...
                self.wfile.write(json.dumps({
                    "status": "ok",
                    "points": ack.get("points", len(trajectory)),
