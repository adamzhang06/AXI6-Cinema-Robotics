...
# --- 2. MOTOR CONTROL THREAD (Velocity Mode) ---
def motor_loop():
    ...
    while state.running:
        ...
        if time.time() - state.last_update > 0.5:
            ...
            if p_speed > 0:
                ...
                # ONLY send a step command if we are starting from 0 or changing direction
                if p_dir != current_dir and p_dir != 0:
                    # Send an "infinite" target so it just cruises
